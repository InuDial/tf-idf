...
use tf_idf::small_str::ArchivedSmallString;
...
use tf_idf::data::{ArchivedLibrary, Library, path_from_bytes, term_freq};
...
fn get_library(folder: impl AsRef<Path>) -> Result<Library, Box<dyn Error>> {
    ...
    let read_dir: Vec<_> = generator.collect();

    // let read_dir: Vec<_> = fs::read_dir(folder)?
    //     .filter_map(|res| match res {
    //         // Ok(entry) if entry.file_type().is_ok_and(|ft| ft.is_dir()) => ,
